...
//@desc get all user profile from user_id  
//@access Public
...
router.get('/user/:user_id', async (req, res) => {
    try {
        ...
        if (!profile) return res.status(400).json({ msg: 'There is no profile for this userid' });
        res.json(profile);
    }
    catch (err) {
        console.error(err.message);
    ...
    try {
        const options = {
            uri: `https://api.github.com/users/${req.params.username
                    ...
                    'githubClientId'
                )}&client_secret=${config.get('githubSecrets')}`,
            method: 'GET',
        ...
        request(options, (error, response, body) => {
            ...
            if (response.statusCode !== 200) {
                return res.status(400).json({ msg: 'No Github Profile find' });
